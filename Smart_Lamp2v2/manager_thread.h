...
//------------------------------------------------------------- UART QUEUE 2 --------------------------------------------------------
void uart2_thread(wiced_thread_arg_t arg){
...
void uart_main1(wiced_thread_arg_t arg){
    ...
    while(1){
        ...
        res = strstr(&message1,"MAC:");
               if(res){

//                   strcpy(MAC_BT_FACTORY,&message1[4]);
//                   set_mac_bt(MAC_BT_FACTORY);
//                   WPRINT_APP_INFO(("ID MAC BT= %s\n",MAC_BT_FACTORY));
        ...
        res = strstr(&message1,"ZOM_BT");
          ...
          res = strstr(&message1,"xd");
            if(res){

            if ( wwd_wifi_get_mac_address( &macx, WWD_STA_INTERFACE ) == WWD_SUCCESS )
               {

                 //  WPRINT_NETWORK_INFO(("WLAN MAC Address : %02X:%02X:%02X:%02X:%02X:%02X\r\n", mac.octet[0],mac.octet[1],mac.octet[2],mac.octet[3],mac.octet[4],mac.octet[5]));
               sprintf(mac_tx,"WMA%02X%02X%02X%02X%02X%02X\n",macx.octet[0],macx.octet[1],macx.octet[2],macx.octet[3],macx.octet[4],macx.octet[5]);
               }
//                wiced_uart_transmit_bytes( WICED_UART_3,mac_tx , sizeof( mac_tx ) - 1 );
                wiced_uart_transmit_bytes( WICED_UART_1, mac_tx, sizeof( mac_tx ) - 1 );
...
//------------------------------------------------------------- MAIN UART 2 --------------------------------------------------------
void uart_main2(wiced_thread_arg_t arg){
}
//------------------------------------------------------------- MAIN UART 3 --------------------------------------------------------
