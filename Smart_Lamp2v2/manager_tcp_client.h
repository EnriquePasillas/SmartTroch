...
//         }
        ...
        sprintf(tx_data,"\nL;%02X:%02X:%02X:%02X:%02X:%02X,%d,%s,%02X:%02X:%02X:%02X:%02X:%02X,%u.%u.%u.%u\r\n",
...
//                     WPRINT_APP_INFO(("Fragmented packets not supported\n"));
                ...
                {
                    //char* resultado;
                    resultado = strstr(rx_data,"A1");   // Busca si existe la palabra A1
                    if (resultado)
                        {
                ...
                {
                    //char* resultado;
                    resultado = strstr(rx_data,"DA");   // Busca si existe la palabra A1
                    if (resultado)
                        {
                ...
                resultado = strstr(rx_data,"N1");   // Busca si existe la palabra N1
                if(resultado)
                {
                ...
                resultado = strstr(rx_data,"P1");   // Busca si existe la palabra P1
                if(resultado)
                {
                   WPRINT_APP_INFO(("ID P1 \n"));
                }
                resultado = strstr(rx_data,"ACK");   // Busca si existe la palabra P1
                if(resultado)
                {
                  WPRINT_APP_INFO(("ID P1 \n"));

                }
                resultado = strstr(rx_data,"NNH");   // Busca si existe la palabra P1
                if(resultado)
                {
                    GPIO_activate=0;
            ...
            resultado = strstr(rx_data,"ota_wifi");   // Busca si existe la palabra P1
               if(resultado)
               {
                   wiced_uart_transmit_bytes(WICED_UART_3, "OTA\r\n", 3);
               }
            resultado = strstr(rx_data,"NFH");   // Busca si existe la palabra P1
              if(resultado)
              {
                  GPIO_falledman=0;
                  set_gpio_menssage(FALLENMEN,GPIO_falledman);
              ...
              resultado = strstr(rx_data,"NSH");   // Busca si existe la palabra P1
                 if(resultado)
                 {
...
//if(flag_tcp==3){
//    if( wiced_network_up(WICED_STA_INTERFACE, WICED_USE_STATIC_IP, &DIIS)==WICED_SUCCESS){
//           yes=1;
//
//           WPRINT_APP_INFO( ("up\r\n") );
//           net_lamp();
...
//    else{
//        WPRINT_APP_INFO( ("down\r\n") );
//        wiced_rtos_deregister_timed_event(&tcp_client_event);
//
//    }
//}
